...
public abstract class IntegrationTestBase : IClassFixture<TestWebApplicationFactory>, IAsyncLifetime
{
    ...
    protected async Task<(HttpClient Client, AccountDto Account, LoginRequest Credentials)> CreateAndAuthenticateUserAsync(
        string email = "test@email.com", string password = "testPassword")
    {
        ...
        var client = Factory.CreateAuthenticatedClient(account!.Id);
