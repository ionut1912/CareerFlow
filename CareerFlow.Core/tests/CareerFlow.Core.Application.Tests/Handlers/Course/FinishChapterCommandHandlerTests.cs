...
public class FinishChapterCommandHandlerTests
{
    ...
    public async Task Handle_ValidCommand_CallsFinishChapterAsyncOnce()
    {
        ...
        _courseServiceMock
            .Setup(s => s.FinishChapterAsync(command.UserId, command.CourseId, command.ChapterId, It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
 
        await _sut.Handle(command, CancellationToken.None);
 
        _courseServiceMock.Verify(
            s => s.FinishChapterAsync(command.UserId, command.CourseId, command.ChapterId, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task Handle_ServiceThrows_PropagatesException()
    {
        var command = new FinishChapterCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid());
        _courseServiceMock
            .Setup(s => s.FinishChapterAsync(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
