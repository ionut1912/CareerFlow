...
public class CourseAssemblerTests
{
    ...
    {
        // FIX: Increased 'A's to 195 so the total length is 204 (195 + 1 space + 8 chars for "LongWord").
        // This ensures the string actually exceeds the 200 character limit to trigger the truncation logic.
        var title = new string('A', 195) + " LongWord";
        var data = new List<ChapterAssemblyModel> { new(1, title, "Core", [CreateSub()], []) };
    ...
    [Fact]
    public void BuildChapters_NullTitle_ThrowsInvalidDataException()
    {
        // FIX: The domain entity Chapter explicitly throws an InvalidDataException if the title is empty.
        // We must assert that this exception happens, rather than expecting it to successfully return "".
        var data = new List<ChapterAssemblyModel> { new(1, null!, "Core", [CreateSub()], []) };

        var exception = Should.Throw<InvalidDataException>(() => CourseAssembler.BuildChapters(data));
        exception.Message.ShouldBe("Titlul capitolului este necesar");
    }
