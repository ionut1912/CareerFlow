...
public class AuthService : IAuthService
{
    ...
    public async Task<Account> LoginWithGoogleAsync(string idToken, CancellationToken cancellationToken = default)
    {
        ...
        if (account == null)
        {
            return await SaveUserAsync(googleUser.Email, googleUser.Name, cancellationToken);
        }
        return account;
    }

    public async Task<Account> LoginWithLinkedInAsync(string authorizationCode, CancellationToken cancellationToken = default)
    {
        ...
        var tokenResponse = await _httpClient.PostAsync("https://www.linkedin.com/oauth/v2/accessToken", tokenReq, cancellationToken);
        tokenResponse.EnsureSuccessStatusCode();

        var tokenData = await tokenResponse.Content.ReadFromJsonAsync<LIToken>(cancellationToken: cancellationToken);
        ...
        var userData = await _httpClient.GetFromJsonAsync<LIUser>("https://api.linkedin.com/v2/userinfo", cancellationToken);
        var account = await _accountRepository.GetAccountByEmailAsync(userData!.email, cancellationToken);

        if (account == null)
        {
            return await SaveUserAsync(userData.email, userData.name, cancellationToken);
        }
